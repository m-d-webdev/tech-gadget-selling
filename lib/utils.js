...
export const handleChangeThem = t => {
...
export const AddCartIdToLocalStorage = (id) => {
  if (typeof (localStorage) != "undefined") {
    let oldIds = localStorage.getItem("cart_ids")
    if (oldIds) { oldIds = JSON.parse(oldIds) } else { oldIds = [] };
    oldIds.push(id);
    localStorage.setItem('cart_ids', JSON.stringify(oldIds))
  }
}

export const DeleteCartIdFromLocalStorage = (id) => {
  if (typeof (localStorage) != "undefined") {

    let oldIds = localStorage.getItem("cart_ids")

    if (oldIds) { oldIds = JSON.parse(oldIds) } else { oldIds = [] };
    oldIds = oldIds.filter(i => i != id);
    console.log({ id });
    console.log({ oldIds });
